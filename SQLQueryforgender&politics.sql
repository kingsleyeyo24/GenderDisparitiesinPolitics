...
SELECT 
    ...
    female_count,
    total_positions,
    ROUND((CAST(female_count AS FLOAT) / total_positions) * 100, 2) AS female_percentage
FROM 
    TotalCount
ORDER BY 
...
SELECT *
FROM [PortfolioProject].[dbo].[GenderandPolitics]
WHERE [Education Budget] IS NOT NULL
